...
$randomString = generateRandomString(10);
 $mess = "recorded ".$ammount." payment of school fees for  admission number ".$adm. "  succsesfully . fee balance is ".$bal;
    $query=mysqli_query($con,"INSERT INTO logs( user , activities ) value('$username','$mess')");
               ...
               $query=mysqli_query($con,"INSERT INTO payment( adm, amount, bal, reference, paytype
                    ) value('$adm','$ammount','$bal','$randomString','CASH')");
...
$amount=$ammount;
        ...
        if ($response && strpos($response, 'success: Payment received Successful') !== false) {
            ...
            if (count($matches) >= 4) {
                ...
                $message = "Dear parent of $fname $lname of  $class, we confirm receipt of Amount Ksh $amount. MPESA Reference $reference and your current balance is Ksh $balance. Thank you";
                ...
                // Generate PDF
                generatePdf($adm, $amount, $balance, $reference);

                // Send SMS
                $apiKey = fetchApiKey($con);
                if ($apiKey) {
                    sendSms($received_phone, $message, $apiKey);
                } else {
                    echo "Error fetching latest API key from the database.\n";
                }
...
function generatePdf($adm, $amount, $balance, $reference) {
    $pdf = new FPDF();
    $pdf->AddPage();
    ...
    $pdf->Cell(36, 10, 'Date', 1);
    $pdf->Cell(36, 10, 'Amount Received', 1);
    $pdf->Cell(36, 10, 'Balance', 1);
    $pdf->Cell(36, 10, 'Payment Method', 1);
     $pdf->Cell(36, 10, 'REFERENCE', 1);
    $pdf->Cell(36, 10, 'Status', 1);
    $pdf->Ln();

    $pdf->SetFont('Arial', '', 10);
    $todayDate = date('Y-m-d');
    $pdf->Cell(36, 10, $todayDate, 1);
    $pdf->Cell(36, 10, $amount, 1);
    $pdf->Cell(36, 10, $balance, 1);
    $pdf->Cell(36, 10, 'MPESA', 1);
     $pdf->Cell(36, 10, $reference, 1);
    $pdf->Cell(36, 10, 'received', 1);
    $pdf->Ln();
